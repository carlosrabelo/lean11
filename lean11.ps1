...
.PARAMETER ProductKey
    ...
    If provided, will be included in autounattend.xml
...
'@
    ...
    # Add product key only if provided by user
    if ($ProductKey) {
        Write-Log "Including product key in autounattend.xml" -Level Info
        $autounattendContent += @'
                <RunSynchronousCommand wcm:action="add">
                    <Order>2</Order>
                    <Path>DISM /Online /Set-Edition:Professional /ProductKey:PRODUCT_KEY_PLACEHOLDER /AcceptEula</Path>
                </RunSynchronousCommand>
'@ -replace 'PRODUCT_KEY_PLACEHOLDER', $ProductKey
    }
